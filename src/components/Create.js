...
const Create = () => {
  ...
  return (
    ...
    {window.innerWidth > 480 && <div style={{textAlign: 'center',padding: '16px',transition: '600ms',backgroundImage: `url(${bg})`,height: 'calc(100vh - 80px)',backgroundSize: '100%'}}>
      ...
      <ButtonTwo onClick={() => setCreate('Char')} onMouseOver={() => {setBg(`${process.env.PUBLIC_URL}/imgs/perso.jpg`) ; setBackground('rgba(0,0,0,.76)')}} style={{fontSize: '40px'}}>Personagem</ButtonTwo>
      <ButtonTwo onClick={() => setCreate('Hist')} onMouseOver={() => {setBg(`${process.env.PUBLIC_URL}/imgs/book.jpg`) ; setBackground('rgba(0,0,0,.76)')}} style={{fontSize: '40px'}}>História</ButtonTwo>
      </div>
    </div>}
